...
module SequenceServer
 with_hits = Job.fetch('blast_2.8.1/blastn')
  ...
  describe 'BLASTN' do
    let 'hsp' do
      report = Report.generate(Job.fetch('blast_2.8.1/blastn'))
  ...
  describe 'BLASTP' do
    let 'hsp' do
      report = Report.generate(Job.fetch('blast_2.8.1/blastp'))
      report.queries.first.hits.last.hsps.first
    end
  ...
  describe 'BLASTX' do
    let 'hsp' do
      report = Report.generate(Job.fetch('blast_2.8.1/blastx'))
  ...
  describe 'TBLASTX' do
    let 'hsp' do
      report = Report.generate(Job.fetch('blast_2.8.1/tblastx'))
      report.queries.first.hits.last.hsps.first
    end
  ...
  describe 'TBLASTN' do
    let 'hsp' do
      report = Report.generate(Job.fetch('blast_2.8.1/tblastn'))
      report.queries.first.hits.last.hsps.first
    end
