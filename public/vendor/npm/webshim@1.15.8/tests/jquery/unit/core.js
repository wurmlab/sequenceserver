...
(function(process) {
  ...
  test("toArray()", function() {
    expect(1);
    deepEqual(jQuery("#qunit-fixture p").toArray(), q("firstp", "ap", "sndp", "en", "sap", "first"), "Convert jQuery object to an Array");
  });
  ...
  test("map()", function() {
    expect(8);
    deepEqual(jQuery("#ap").map(function() {
      return jQuery(this).find("a").get();
    }).get(), q("google", "groups", "anchor1", "mark"), "Array Map");
    deepEqual(jQuery("#ap > a").map(function() {
  ...
  test("jQuery.extend(Object, Object)", function() {
    ...
    ret = jQuery.extend(true, {foo: []}, {foo: [0]});
  ...
  if (jQuery.sub) {
    test("jQuery.sub() - Static Methods", function() {
      ...
      Subclass.fn.extend({"subClassMethod": function() {
          return this;
        }});
      ...
      SubSubclass.fn.extend({"subSubClassMethod": function() {
          return this;
        }});
      SubSubclass["setup"]({
        "locale": "es_MX",
      ...
      jQuery.each(selectors, function(i, selector) {
        jQuery.expandedEach({
          ...
          jQuery.each(contexts, function(i, context) {
            ...
            deepEqual((function(var_args) {
              return jQuery.fn[method].apply(jQuery(selector, context), arguments).subclassMethod;
            })(arg), undefined, "jQuery" + description + " doesn't have Subclass methods");
            deepEqual((function(var_args) {
              return jQuery.fn[method].apply(jQuery(selector, context), arguments).subclassSubclassMethod;
            })(arg), undefined, "jQuery" + description + " doesn't have SubclassSubclass methods");
            deepEqual(Subclass(selector, context)[method](arg).subclassMethod, Subclass.fn.subclassMethod, "Subclass" + description + " has Subclass methods");
            deepEqual(Subclass(selector, context)[method](arg).subclassSubclassMethod, undefined, "Subclass" + description + " doesn't have SubclassSubclass methods");
            deepEqual(SubclassSubclass(selector, context)[method](arg).subclassMethod, Subclass.fn.subclassMethod, "SubclassSubclass" + description + " has Subclass methods");
            deepEqual(SubclassSubclass(selector, context)[method](arg).subclassSubclassMethod, SubclassSubclass.fn.subclassSubclassMethod, "SubclassSubclass" + description + " has SubclassSubclass methods");
          });
        });
      });
    });
