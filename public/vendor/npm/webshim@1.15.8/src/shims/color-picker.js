...
(function(process) {
  (function($, version) {
    ...
    var Slider = function(bar, options) {
      ...
      bind.call($this, draw);
        ...
        ColorValuePicker = function(picker, color, bindedHex, alphaPrecision) {
          var $this = this,
              ...
              colorChanged = function(ui, context) {
                ...
                if (alpha && context != alpha.get(0))
                  alpha.val(all != null ? mPrecision((all.a * 100) / 255, alphaPrecision) : '');
          ...
          elemExtend(true, $this, {destroy: destroy});
    ...
    var Color = $.wsjPicker.Color,
        List = $.wsjPicker.List,
        ColorMethods = $.wsjPicker.ColorMethods;
    $.fn.wsjPicker = function(options) {
      var $arguments = arguments;
      return this.each(function() {
        var $this = this,
            settings = $.extend(true, {}, $.fn.wsjPicker.defaults, options);
        ...
        var container = null,
            ...
            setAlpha = function(obj, alpha) {
              ...
              if (alpha > 0 && alpha < 100) {
                obj.css({opacity: mPrecision(alpha / 100, 4)});
            ...
            initialize = function() {
              ...
              activePreview = preview.find('.Active').eq(0).css({backgroundColor: hex && '#' + hex || 'transparent'});
              currentPreview = preview.find('.Current').eq(0).css({backgroundColor: hex && '#' + hex || 'transparent'}).on('click', currentClicked);
  ...
  webshims.register('color-picker', function($, webshims, window, document, undefined, options) {
    ...
    picker.commonColorInit = function(data) {
      ...
      popover.element.on({wspopovershow: function() {
    ...
    picker.color.showPickerContent = (function() {
      ...
      var createPicker = function(data) {
        ...
        if (!jPickerApi) {
          jpicker.empty().wsjPicker({}, function(color) {
            if (curData) {
              pickerSet('setInputColor', curData);
            }
          }, false, function(color) {
            if (curData) {
      ...
      return function(data) {
        ...
        value += (data.alpha && data.alpha.length) ? $.wsjPicker.ColorMethods.intToHex((data.alpha.prop('value') || 1) * (255 / (data.alpha.prop('max') || 1))) : 'ff';
        ;
