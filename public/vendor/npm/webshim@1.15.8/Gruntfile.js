...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    copy: {
      main: {},
      archive: {files: [{
          src: 'webshims-stable.zip',
          dest: 'webshims-<%= pkg.version %>.zip'
        }]}
    },
    compress: {main: {
        options: {archive: 'webshims-stable.zip'},
        files: [{
          expand: true,
          cwd: 'demos/',
          src: ['**'],
    ...
    clean: {build: {src: ['js-webshim/', 'src/shims/combos', 'webshims-*.zip']}},
    ...
    bytesize: {all: {src: [MINPATH + '/polyfiller.js']}},
    uglify: {
      options: {
        beautify: {ascii_only: true},
        preserveComments: 'some',
        compress: {
          global_defs: {"WSDEBUG": false},
          dead_code: true
        }
      },
      demo: {
  ...
  grunt.registerTask('webshimscombos', 'create combos from polyfiller.js.', function() {
    ...
    grunt.util.spawn({
      cmd: phantomjs.path,
      args: ['build/combobuild.js', 'build/build.html']
